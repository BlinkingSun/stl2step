...
### Added
- **Torus face on a shared cylinder rim** (`src/refit_torus.cpp` stage T + `src/refit_build.cpp` U-3; D-140-6, D-140-11). S04's boss-top round ships as `Geom_ToroidalSurface` (Rmaj=7, Rmin=3) in a valid closed solid by writing the missing v-isoline pcurves on the reused rim TShape (`Geom2d_Line((u0−f, v), (1,0))`), both seam pcurves at `u0` / `u0+2π`, and the c-rev wire; bind site stays `circle-on-torus`. S19 mouth rounds and real-part tori stay unbuilt (expected-red `S19.mouth-round-not-built`; B1 percentage-of-R band registered as `b1.percentage-accept-band`).
...
- **J6/free-edge containment, landed dormant** (D-140-4; SPEC-reverts-cycle2 §2). Three edits: `regionClosureHealEligible` (`refit_build.cpp:6690`) drops its `builtAs == BuiltAs::Single` clause and becomes `regionBuiltAnalytic(...)`, so `Single | Seamed360 | TwoHalves` are all eligible; closure-heal reachability widens from `fallbackGuardPass == 0` to `fallbackGuardPass <= 2`, which is the guard's own existing bound at `:15281` (`fallbackGuardPass < 2`) and not a new constant; and the J6 `if (j6UncollapsePass > 0)` handoff runs **one** containment round — smallest owning region first by `(nTri ASC, rid ASC)`, `eprimeFill[rid] = 1`, `builtAs = ExplodedToFacets`, then `goto try_rebuild` — before `return false` stays the terminal outcome. A stall restores **both** `eprimeFill` and the snapshotted `builtAs`, and both counters are restored on every `return false` out of `buildFaces`, so **containment is reported only for a component that shipped**. No new numeric literal in `src/` or `include/`.
- **Two RESULT keys, `smoothContainedRegions` and `smoothContainedTriangles`** (D-130-10 declaration; `RefitStats::containedRegions` / `containedTriangles`, summed in `Converter::run`). Incremented at exactly one place — the J6-handoff demotion, after the admitted set shrank. Never on the seamed360-fail explode path, never on the D-S3-54 closure heal (which already fires at `3bf0575` and keeps reporting itself through `DIAG_HEAL` / `DIAG_HEAL_SUM`). The warning `smooth: contained N region(s) to facets -- component built` is emitted only when `N > 0`. `DIAG_CONTAIN` is emitted only from the J6 handoff, under `diagP2Enabled()`, with a real `freeBefore > 0`.
  **The site is dormant and is proved dormant, not hoped dormant:** it needs `j6UncollapsePass > 0` and `freeE > 0` together, and no artefact this project owns reaches that — 32 corpus fixtures emit zero `J6:` lines post-heal; S16-R2-ChainUnstable emits one `J6:` line but no uncollapse, so `j6UncollapsePass` stays 0; Body11 is excluded by `mv.nTri < 10000` (untouched, row `revert.j6-ntri-10000`); shelf_bracket and Body244 close; grip and vase whole-component caller-R2 before P2; and the private Body9/12/18/20 set is closed at `3bf0575` with `freeEdges = 0`. Both keys read `0` and `DIAG_CONTAIN` count is `0` on every one of them, measured. **Sunset (D-140-4 §1(5)): if D-140-5 cannot construct `S18_uncontainable_rim` or any artefact reaching `j6UncollapsePass > 0`, the J6 handoff is reverted, not carried.** Eligibility and reachability stay either way — they widen a heal path that is live.
...
- **Two real-export corpus fixtures**, `shelf_bracket` (2340 tris; 32 planes + 20 cylinders + 735 facet faces in one valid closed solid, `volumeDeltaPct = 0.000142`) and `Body244` (6268 tris; 364 planes + 178 cylinders, `volumeDeltaPct = 0.000099`, `rid=1` explodes to 148 facets under expected-red `Body244.cyl360.rid1`). shelf_bracket is the analytic-plus-facet-island proof on real geometry that `S17_hole_vs_island` was to assert synthetically; S17 is withdrawn this cycle under expected-red `S17.cap-claimed-by-a2` (D-140-4 §4).
...
### Deferred
- **Torus engine stage T + U-3 face sharing the cylinder rim** (D-140-6, D-140-11): not in `5d546dd` (`git log --oneline 5d546dd | grep -c 'transactional reclaim'` = 0). Math and S04/S19 fixtures landed; no `Geom_ToroidalSurface` face ships. `S04.torus-not-built` / `S19.mouth-round-not-built` stay.
