...
- Gate fixture `linkage_bores_chamfer` (`linkage_bores_chamfer.stl`, `.expected.json`) for the Mastercam linkage plate: N-gon bores plus the hole-mouth 45° chamfer. Its contract is owned by `gate_130` (`tests/gates/gate_130.py`, the 1.3.0 battery gate, registered expected-red until the battery closes).
...
- **Radius truth census** (`radiusDrift` in RESULT, one `DIAG_RADIUS` line per built cylinder under `STL2STEP_P2_DIAG`): the radius each built cylinder ships, against the least-squares radius of its own claimed vertices about its own axis, with the covered span and both residuals. Minimising the squared radial residual at a fixed axis gives the mean radial distance in closed form, so there is no solver and no constant. Measurement only: a face whose radius the mesh does not support is valid, watertight and invisible to every other gate, and this is the first number that names it.
- **Torus-band census** (`DIAG_TORUSBAND`, `DIAG_TORUSBAND_SUM`): the fillet strips whose neighbouring strips' axes turn, which is what separates a slice of a real cylinder from a slice of a torus. The comparison tolerance is the angle each strip's own fit residual subtends at its own radius — the mesh measuring itself, not a chosen constant.
- **Edge-class census** (`edgeClasses` in RESULT; one `DIAG_EDGECLASS` line per edge under `STL2STEP_P2_DIAG`): every edge two analytic faces share is classified on the SHIPPED shell against the tolerance that edge finally records and the mesh tolerance cap of the region behind each face. An edge is tier 1 (`analytic`) when its own curve is a line, circle or ellipse and the closed-form bind-site supremum exists on both surfaces; tier 2 (`polylineTier2`) when it is a mesh edge the chain machinery did not collapse — counted, never absorbed; and `unhandled` otherwise, which is the case an analytic edge shipping without a closed-form supremum falls into. Two further counters, `overTol` and `overCap`, hold the two numeric promises: no edge deviates from a surface by more than the tolerance it declares, and no declared tolerance exceeds the mesh's own cap. An analytic face is recognised by the identity of its surface handle (a fitted region surface, never a per-triangle facet plane), so the census reads the artefact rather than construction bookkeeping, and construction is unchanged.
- **`smoothBuiltCones` in RESULT**: the conical faces a build actually shipped, counted off the written shape the way `smoothBuiltCylinders` is. Reads 0 on every part with no chamfer frustum. A new RESULT key, so a comparison of RESULT across this change is made modulo it.
- **Mesh quantization floor** (`DIAG_QUANTFLOOR`, print-only): the resolution of the number format a mesh file was written in, at that mesh's own coordinate magnitude — for a binary STL the binary32 ulp at the largest |coordinate| in the file times √3/2 (three independent per-axis half-ulp roundings, as a radius); for an ASCII STL the printed-decimal grid at the same magnitude, with the writer's precision read off as the largest significant-digit count any coordinate token carries. Measured from the file rather than from the loaded doubles, which no longer say what format they arrived in, and carried into the engine's own millimetres. An unreadable file yields 0, which certifies nothing.

### Changed
- **I7 loop closure**: a boundary walk that leaves a split vertex and returns to the same vertex is a cycle (`walkFrom` stops at the first split it reaches, so `front == back` can only mean it closed on itself). If that vertex is incident to ≥ 3 regions/islands it is a chain endpoint (I8), so the cycle is split there — and at the ring's lowest vertex id, I5's origin, when that is the only junction — into open chains that still form a complete loop (I7). Otherwise the cycle is recorded as one closed chain. A region's whole outer loop whose only neighbour is an island is no longer one `closedLoop=false` chain, and the junction is no longer interior to it.
- **RESULT `warnings` order**: warnings are raised from a thread pool of component builds, so their arrival order was scheduler state rather than a property of the mesh — the same 34 strings were observed in six different orders on one fixture with identical STEP output. The list handed back is now sorted, which makes the RESULT line deterministic; the live log callback still sees each warning at the moment it is raised.
...
- **Law-band extent**: a recognised law band now owns every adjacent unclaimed triangle whose vertices lie on the cylinder the band has already certified. The equal-theta chain law is how a band is recognised; it is not what fixes the band's extent — a wall triangulated as a staggered strip has a mixed generator pitch, so the chain stops at the first regular run even though the rest of the ring sits on the radius the band measured. A triangle is admitted only inside `max(the band's own certified residual, the mesh file's quantization floor)`, never a bounding-box budget; after each round the band's cylinder is refit by least squares over its members **with the axis direction among the unknowns**, and a round whose refit leaves the certificate is rolled back whole. A band that absorbs nothing is left untouched. On the bundled linkage plate this closes the R 8.5 through-bore (6 → 139 triangles, deviation 6.5e-6 mm against a floor of 1.3e-5) and completes both R 30 profile arcs to their ground-truth extent (71 → 75, 70 → 80), so the plate ships its hole-B walls, both 45° chamfer frustums and the cross bore as analytic faces; the cross bore reaches 108 of its 279 ground-truth triangles, the rest being pinned by eight mesh vertices that sit 7.1e-4 mm inside the exact bore. On the Handle pickup fixture two R≈3 bands take three triangles between them, each inside its own band's already-certified residual: 918 → 916 faces, |V_step − V_mesh| 10.309374 → 10.115732 mm³, both radii moving to the least-squares value over the enlarged band, watertight and valid throughout.
- **Virtual generators**: a law band's chain may be built on the lines where two adjacent facet PLANES meet rather than on the mesh edges the tessellation happened to draw. On a tessellated cylinder that line is the inscribed polygon's own corner line — exactly on the surface and exactly parallel to the axis — so a wall whose two rings sit at alternating azimuths, with no axis-parallel edge at most of them, chains at every fold the mesh resolves. The lines find their own direction (the direction the most of them agree with, inside the band's existing scale-relative parallelism tolerance) rather than trusting a three-facet normal PCA; a fold counts only when each triangle's far corner stands off its neighbour's plane by more than the mesh file's own quantization, below which the two planes' cross product is round-off; and where a generator is missing the chain reads an exact integer multiple of the pitch, so equal-theta is applied to the reduced pitches instead. In equal-theta's place such a band must carry the chain law alone (radius consistency with no disjunction), lie on its cylinder within the file's quantization rather than a part-scale budget, and be one the mesh can tell from a plane — without that last test a flat wall between two equal folds fits a cylinder exactly and ships as one. A chain the mesh drew is never displaced: the virtual path runs only where the mesh-edge chain refused, and a band grows by the law it was certified under. On the bundled linkage plate the four R 3 slot ends are recognised for the first time (each wall 9.5e-7 mm from its cylinder against a 1.3e-5 mm floor), the R 8.5 through-bore is claimed as a full 92-generator closed chain rather than a four-side fragment grown afterwards, and the cross bore reaches 214 of its 279 ground-truth triangles. On the Handle pickup fixture the face, plane and cylinder counts are unchanged (916 / 795 / 121) while two more rims ship as circles and |V_step − V_mesh| falls 10.115732 → 9.732831 mm³; every other corpus part is byte-identical.
