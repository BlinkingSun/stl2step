...
### Changed
- **I7 loop closure**: a boundary walk that leaves a split vertex and returns to the same vertex is a cycle (`walkFrom` stops at the first split it reaches, so `front == back` can only mean it closed on itself) and is recorded as a closed chain. A region's whole outer loop whose only neighbour is an island is no longer emitted as one `closedLoop=false` chain. Corpus region/chain counts are unchanged except those loops.
